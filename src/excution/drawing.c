...
/*   Updated: 2026/03/05 14:06:41 by jaeklee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    int y = 0;
    int ceil_color = (game->ceiling.r << 24) | (game->ceiling.g << 16) | (game->ceiling.b << 8) | 255;
...
// 벽 텍스처 그리기
static void draw_wall(t_ray *ray, t_game *game, int x, mlx_texture_t *tex, int tex_x)
{
    int y = ray->draw_top;
    int tex_y;
    int color;
...
{
    int y = draw_bottom + 1;
    int floor_color = (game->floor.r << 24) | (game->floor.g << 16) | (game->floor.b << 8) | 255;
